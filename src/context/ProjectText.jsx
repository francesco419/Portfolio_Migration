    ...
    data: [
      ...
      },
      {
        ...
        info: [
          {
            subtitle:
              '**디자이너와 Figma를 통한 협업**을 통해 시각적 웹 개발을 원활하게 진행했습니다.\n Figma를 활용하여 디자인 파일을 **공유하고 실시간으로 협업**하며, **개발과 디자인 간의 원활한 의사소통**을 도모하여 프로젝트의 일관성과 효율성을 높였습니다.',
...
이메일 주소, 이름 , 지역, 사진 의 **데이터를 API로 부터 제공**받아 회원가입시 추가 정보 입력은 최소화 했습니다.
          ...
          {
            subtitle:
              '**Kakao Map API**를 활용하여 지리적 **데이터 시각화**를 강화했습니다.\n 다양한 지도 기능을 통해 사용자에게 직관적이고 효과적인 지리 정보를 제공하며, **애플리케이션의 사용자 경험을 향상**시켰습니다.',
...
\`\`\`
          ...
          {
            subtitle:
              '프론트와 백엔드의 **서버를 분리배포**하여 시스템 유연성을 향상시켰습니다.\n 클라이언트와 **서버 간의 독립성을 유지**하면서, 효율적인 확장과 업데이트를 가능케 하여 **개발 생산성과 안정성**을 동시에 향상시켰습니다.',
...
`
          ...
          {
            subtitle:
              '**AWS 서비스**를 사용하여 안정적이고 확장 가능한 개발 환경을 조성했습니다. \n다양한 **클라우드 기술을 활용**하여 **서버 관리 및 인프라 구축**을 효율적으로 수행하고, 안전한 데이터 저장 및 전송을 보장하였습니다.',
...
**multerS3**를 사용해 multer기능을 이용하여 S3에 파일을 전송을 가능케 했습니다. 또한 AWS와의 연결보안을 위해 **AWS IAM**을 사용, IAM에 따른 역할 엑세스 지정과 해당 엑세스 키를 사용해 메인 ID의 사용을 줄여 **보안을 강화**했습니다.
          ...
          {
            subtitle:
              'SCSS를 사용하여 Interactive한 개발을 경험했습니다. **SCSS를 활용하여 스타일의 효율적 구성과**, SVG와 결합하여 **다양한 그래픽 요소를 삽입한 상호작용성**을 부여했습니다. \n이를 통해 독특하고 다이내믹한 웹 페이지를 만들어 사용자 경험을 높였습니다.',
...
**DOM 조작을 최소화**와 코드의 재사용성을 가져가면서 원하는 효과를 구현하기 위해 스타일시트를 통한 문제해결 방식을 사용했습니다. **SCSS를 사용하고 있었기에 보다 직관적이고 간략한 해결방법을 모색**, 수집된 유용한 정보들을 토대로 아래 **SCSS 구문으로 원하는 효과를 구현**해냈습니다.
    ...
    data: [
      {
        ...
        info: [
          {
            subtitle: 'TypeScript 정적코드 사용 계기',
            detail: `페이지 변경 중, 새로운 기능 제작 완료 과정에서 알 수 없는 오류가 발생 하였습니다.\n가장 큰 문제는 오류 발생 원인에 대한 확실한 메세지가 제공되지 않고 너무 포괄적인 메세지 제공으로 어느 한 부분에 오류가 발생되었는지 바로 확인할 수 없었습니다. 모호한 오류 메세지를 확실하게 분석하기 위해서 꽤 오랜시간 자료 탐색을 하였고 여러 답안 중, "타입 오류로 인한 문제발생 가능성이 크다"라는 정보를 바탕으로 문제발생 위치를 탐색하였습니다.
              \n\n복잡한 수학 계산식을 포함한 기능이였기에 사용된 모든 함수의 과정 및 결과를 빠짐없이 체크하였고 그중 한 부분에서 number타입의 결과를 도출해야하지 않고 string타입의 결과를 도출하는 부분을 발견하였습니다. 매우 간단하게 수정하고 바로잡을 수 있는 문제였지만 생각보다 긴 시간을 소비하게 되었습니다.
              \n\n이 문제 이전에 TypeScript의 존재를 알고 있었지만 JavaScript로도 충분하다고 생각하여 배재하고 개발 공부를 이어나갔습니다. 하지만 해당 경험을 바탕으로 TypeScript에 대해 진지하고 알아보고 차기 프로젝트에 적극 활용함과 동시에 개발 효율성을 높일 있는 기술을 탐색하고 학습하는 적극적인 자세를 가지게 되었습니다.`
          },
          {
            subtitle:
              '다양한 레이아웃을 구현능력을 키우고 디자인 패턴에 관심을 가짐',
            detail: `웹페이지란 시각적으로 사람을 끌어들일 수 있어야 한다고 생각합니다. 매력적인 UI는 긍정적인 사용자경험을 제공할 수 있다고 생각하여 자주 레이아웃을 변경하며 인터넷에서 찾을 수 있는 다양한 레이아웃을 구현해보며 자연스럽게 디자인 패턴에 대해 관심을 가지게 되었습니다.\n\n 매력적인 디자인 구현과 동시에 HTML마크업, CSS스타일링, 클린코드를 통한 효율적인 기능에 대해 고민하며 매번 발전하고자 노력합니다.`
          },
          {
            subtitle: '개발 환경 개선을 통한 사용자 경험 개선',
            detail: `항상 내가 작성하는 코드의 개선과 다양한 디자인 패턴 구현을 통해 사용자 경험을 높이는 목표를 가지고 개발에 임했었습니다. 하지만 프로젝트를 거듭하여도 크게 달라지지 않는 결과물에 "내가 어떤 변화를 가져와야 개선을 할 수 있고, 이를 통해 지속적인 성장을 도모할 수는 없을까?"라는 생각을 했습니다.
            \n\n따라서 평소 등한시 했던 개발 환경에 변화를 가져오자 결정하였고, 새롭게 떠오르는 기술인 Vite를 선택하였습니다. Vite 공식문서를 정독하며 소홀했던 기초 지식들 예를 들어, 번들링, 모듈러, 빌드 등을 깊게 학습하며 개발에 대한 재미를 더욱 깊게 가져갈 수 있었습니다. 포트폴리오 페이지를 Vite 환경으로의 이전 및 여러 설정을 통해 사용자가 체감할 수 있는 긍정적인 개발을 경험하였습니다. 더 좋은 퍼포먼스를 보여주기 위해 제공하는 페이지에 대해 분석 및 개선하는 재미를 가지게 되어 능동적으로 깊게 학습하게 되었습니다.`
          },
          {
